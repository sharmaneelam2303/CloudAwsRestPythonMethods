...
def __getTotalCountForAwsAutoScalingGroups():
    ...
    for region in ec2client.describe_regions()['Regions']:
        ...
        if reg not in ignored_regions:
            autoscaling = boto3.client('autoscaling', region_name=reg, aws_access_key_id=aws_access_key_id,
...
def __getTotalCountForAwsEbsVolume():
    ...
    for region in ec2client.describe_regions()['Regions']:
        reg = region['RegionName']
        if reg not in ignored_regions:
            ec2 = boto3.client('ec2', region_name=reg, aws_access_key_id=aws_access_key_id,
                               aws_secret_access_key=aws_secret_access_key)
...
def __getTotalCountForAwsInstances():
    ...
    for region in ec2client.describe_regions()['Regions']:
        reg = region['RegionName']
        if reg not in ignored_regions:
            ec2 = boto3.resource('ec2', region_name=reg, aws_access_key_id=aws_access_key_id,
                                 aws_secret_access_key=aws_secret_access_key)
...
def __getTotalCountForAwsLambdaFunction():
    ...
    for region in ec2client.describe_regions()['Regions']:
        reg = region['RegionName']
        if reg not in ignored_regions:
            lambdaClient = boto3.client('lambda', region_name=reg, aws_access_key_id=aws_access_key_id,
                                        aws_secret_access_key=aws_secret_access_key)
            try:
...
def __getTotalCountForAwsLoadBalancer():
    elbv2total = 0
    elbtotal = 0
    ec2client = boto3.client('ec2', region_name='us-east-1', aws_access_key_id=aws_access_key_id,
                             aws_secret_access_key=aws_secret_access_key)
    for region in ec2client.describe_regions()['Regions']:
        reg = region['RegionName']
        if reg not in ignored_regions:
            client = boto3.client('elbv2', region_name=reg, aws_access_key_id=aws_access_key_id,
                                  aws_secret_access_key=aws_secret_access_key)
            response = client.describe_load_balancers()
            for res in response['LoadBalancers']:
                elbv2total = elbv2total + 1
            client = boto3.client('elb', region_name=reg, aws_access_key_id=aws_access_key_id,
                                  aws_secret_access_key=aws_secret_access_key)
            response = client.describe_load_balancers()
...
def __getTotalCountForAwsRds():
    ...
    for region in ec2client.describe_regions()['Regions']:
        reg = region['RegionName']
        if reg not in ignored_regions:
            rdsClient = boto3.client('rds', region_name=reg, aws_access_key_id=aws_access_key_id,
                                     aws_secret_access_key=aws_secret_access_key)
...
def __getTotalCountForEKSCluster():
    ...
    for region in ec2client.describe_regions()['Regions']:
        reg = region['RegionName']
        if reg not in ignored_regions:
            eksClient = boto3.client('eks', region_name=reg, aws_access_key_id=aws_access_key_id,
                                     aws_secret_access_key=aws_secret_access_key)
    ...
    for region in ec2client.describe_regions()['Regions']:
        ...
        if reg not in ignored_regions:
            ...
            for cluster in clusters['clusters']:
                ...
                print(profiles)
                for profile in profiles['fargateProfileNames']:
                    total = total + 1
    return total
